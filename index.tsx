...
function displayResults(imagePrompts: string[], videoPrompts: string[]) {
    ...
    // Separator logic: visible only if both sections are meant to be shown together and both have actual content
...
async function generateCreativeIdeas(keywords: string, type: 'image' | 'video') {
    ...
    try {
        ...
        // Split by newline, then filter and map
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (downloadPromptsButton) {
        downloadPromptsButton.onclick = () => {
            ...
            if (imageItems.length > 0 && !imagePromptOutputContainer.classList.contains('hidden')) { // Check if section is visible
            ...
            if (videoItems.length > 0 && !videoPromptOutputContainer.classList.contains('hidden')) { // Check if section is visible
            ...
            let content = sections.join('\n\n\n'); // Use more newlines to clearly separate sections
    ...
    function handleCheckboxExclusivity(checkbox1: HTMLInputElement, checkbox2: HTMLInputElement, label1: HTMLLabelElement, label2: HTMLLabelElement) {
        if (checkbox1.checked) {
            ...
            // label2.setAttribute('aria-checked', 'false'); // This will be handled by setupCustomCheckbox's change event on checkbox2
        }
        // label1.setAttribute('aria-checked', checkbox1.checked.toString()); // Handled by setupCustomCheckbox
        // Ensure both labels are updated based on their checkbox's current state
    ...
    if (highlyDetailedCheckbox && highlyDetailedLabel && simpleCheckbox && simpleLabel) {
        ...
        highlyDetailedCheckbox.addEventListener('change', () => {
            if (highlyDetailedCheckbox.checked) {
                ...
                simpleLabel.setAttribute('aria-checked', 'false'); // Directly update related checkbox aria
            }
        });
        ...
        simpleCheckbox.addEventListener('change', () => {
            if (simpleCheckbox.checked) {
                ...
                highlyDetailedLabel.setAttribute('aria-checked', 'false'); // Directly update related checkbox aria
            }
        });
    }
